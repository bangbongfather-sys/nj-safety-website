...
/**
 * /admin/contact/edit — WYSIWYG editor for the 문의 page.
 *
 * Mirrors /admin/about/edit line-for-line but renders ContactPage.
 * Same dict files (ko.json / en.json) — Contact copy lives under
 * `contact.*` so every editor primitive (EditableText, EditableImage,
 * ImageSlotPanel, FloatingToolbar, autosave, manual publish, discard)
 * is reused.
 *
 * Behavioural notes specific to ContactPage in edit mode:
 *   • The inquiry-type cards render as plain <div> instead of <button>
 *     so admin clicks don't switch the active selection while editing
 *     the card text.
 *   • All FAQ items render expanded so the admin can edit answers
 *     without clicking each Q open.
 *   • The submit button is disabled — no accidental form submissions.
 */
...
function setIn(obj: unknown, path: (string | number)[], value: unknown): unknown {
  ...
  // current obj isn't already an array, create one — otherwise deep
  // edits into missing array fields would create { "0": {...} } objects
  // instead of [{...}] arrays, breaking later .map() calls.
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (koChanged) {
        ...
        const r = await ghPutFile(pat, KO_PATH, koText, 'chore(contact): inline edit ko', load.koSha);
        lastSha = r.commitSha;
        if (r.contentSha) nextKoSha = r.contentSha;
      }
      if (enChanged) {
        const enText = JSON.stringify(enDraft, null, 2) + '\n';
        const r = await ghPutFile(pat, EN_PATH, enText, 'chore(contact): inline edit en', load.enSha);
        lastSha = r.commitSha;
    ...
    <div className="ed-root">
      <div className="ed-bar">
        <div className="ed-bar-l">
          ...
          <span className="ed-bar-mode">CONTACT · 문의</span>
      ...
      <div className="ed-page">
        ...
        <main>
          <ContactPageView locale={active} dict={activeDict!} editor={editor} />
        </main>
